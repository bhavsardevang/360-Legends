...
open class TagView: UIView {
    ...
    open var isBackgroundColorChange: Bool = false
    ...
    open override func layoutSubviews() {
        //chage made
        if isNoImage {
            icon.frame = CGRect(x: 2, y: 0, width: 0, height: 0)
        } else {
            if font.pointSize == 12.0 {
                icon.frame = CGRect(x: marginLeft, y: marginTop + 4, width: 20, height: 20)
            } else {
                icon.frame = CGRect(x: marginLeft, y: marginTop + 8, width: 20, height: 20)
            }
        }
        ...
        icon.tintColor = .clear//tintColor
        ...
        //change made
        textLabel.frame = CGRect(x: textLeft, y: marginTop + 8, width: 100, height: 20)
        ...
        textLabel.font = font
        textLabel.textColor = .black//tintColor
        textLabel.sizeToFit()
        //change made
        let tagHeight = Int(max(textLabel.frame.height,14)) + marginTop*5 //(2)
        let tagWidth = textLeft + Int(max(textLabel.frame.width,16)) + marginLeft
        
        let dismissLeft = Int(icon.frame.origin.x) + Int(icon.frame.width) + marginLeft/2
        dismissView.frame = CGRect(x: 0, y: 20, width: dismissLeft, height: tagHeight)
        var newIndexValue:Int = 20
        if isBackgroundColorChange {
            newIndexValue = 0
        } else {
            newIndexValue = 20
        }
        frame = CGRect(x: Int(frame.origin.x), y: Int(frame.origin.y + CGFloat(newIndexValue)), width: tagWidth, height: tagHeight)
